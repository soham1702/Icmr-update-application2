...
export class TransactionsRoutingResolveService implements Resolve<ITransactions> {
  constructor(protected service: TransactionsService, protected router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ITransactions> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((transactions: HttpResponse<Transactions>) => {
