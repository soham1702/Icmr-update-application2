...
export class SupplierService {
  ...
  protected convertDateFromClient(supplier: ISupplier): ISupplier {
    return Object.assign({}, supplier, {
      lastModified: supplier.lastModified?.isValid() ? supplier.lastModified.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((supplier: ISupplier) => {
        supplier.lastModified = supplier.lastModified ? dayjs(supplier.lastModified) : undefined;
      });
    }
    return res;
  }
